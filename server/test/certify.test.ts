...
const WRITES: Write[] = [
  ...
  { name: 'team add', method: 'POST', path: s => `/api/events/${s.eventId}/teams`, body: () => ({ name: 'Hillcrest', color: 'green' }) },
  { name: 'team patch', method: 'PATCH', path: s => `/api/events/${s.eventId}/teams/${s.teamA}`, body: () => ({ name: 'Renamed' }) },
  { name: 'team delete', method: 'DELETE', path: s => `/api/events/${s.eventId}/teams/${s.teamB}` },
  ...
  { name: 'athlete scoring', method: 'POST', path: s => `/api/events/${s.eventId}/athletes/scoring`, body: s => ({ ids: [s.a1], scoring: true }) },
  { name: 'athlete delete', method: 'DELETE', path: s => `/api/athletes/${s.a1}` },
  { name: 'roster sync', method: 'POST', path: s => `/api/events/${s.eventId}/roster/sync` },
