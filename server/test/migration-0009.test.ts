  ...
  it('adds a nullable far column and leaves the event row otherwise intact', async () => {
    ...
    try {
      await db.run(sql`insert into events
        ...
        values (1, 'Fall Duals', '2026-10-03', 2, '0420', 'setup', 'live', 1, '2026-10-03T15:00:00.000Z')`)
      ...
      expect(row).toMatchObject({ id: 1, name: 'Fall Duals', mat_count: 2, mat_code: '0420', far: null })
    } finally {
      cleanup()
    }
  })
