...
rulesetRoutes.get('/events/:eventId/rulesets', requireAdmin, async c => {
  const db = c.get('ctx').db
  return c.json(await db.select().from(rulesets).where(eq(rulesets.eventId, Number(c.req.param('eventId')))).orderBy(asc(rulesets.id)).all())
})
