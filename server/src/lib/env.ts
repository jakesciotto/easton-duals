 ...
 * score typed in dev lands on the record the gym reads. DUALS_DEV_REMOTE=1 opts back into
...
export function applyDevDefaults(env: Record<string, string | undefined>): string | null {
  // The old spelling stays accepted for one release, because a shell may still export it.
  if (env.DUALS_DEV_REMOTE === '1' || env.DUELS_DEV_REMOTE === '1') return null
